...
class EasyBlogViewBlogs extends EasyBlogAdminView
{
	public function display($tpl = null)
	{
		...
		foreach ($rows as &$row) {
			...
			// Test if this blog post is posted in a group
			if (EB::groups()->useGroups() && $post->source_id && $post->source_type == EASYBLOG_POST_SOURCE_JOMSOCIAL_GROUP) {

				$type = EB::helper('Groups')->getGroupSourceType();
				$blog->contributionDisplay 	= EB::helper('Groups')->getGroupContribution($post->id, $type, 'name') . ' (' . $type . ')';
			}

			if (EB::helper('Event')->useEvents() && $post->source_id && $post->source_type == EASYBLOG_POST_SOURCE_JOMSOCIAL_EVENT) {
				$type 	= EB::helper('Event')->getGroupSourceType();
				$post->contributionDisplay 	= EB::helper('Event')->getGroupContribution($post->id, $type, 'name') . ' (' . $type . ')';
			}

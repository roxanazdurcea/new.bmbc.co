...
class EasyBlogPost extends EasyBlog
{
	...
	public function postSave()
	{
		...
		// Add activity stream for EasySocial, regardless if it is being published or not, we should let the client check this.
	...
	public function savePost()
	{
		...
		// $tmpFile =  JPATH_ROOT . '/tmp/eb_debug.log';

		// $debugfile = fopen($tmpFile, "a");
		// fwrite($debugfile, $out . "\n\n\n**************************************\n\n\n");
		// fclose($dedugfile);
	...
	public function validateFields()
	{
		foreach ($this->categories as $categoryId) {
			...
			foreach ($fields as $field) {
				...
				if ($field->required && (!isset($this->fields[$field->id]) || empty($this->fields[$field->id]))) {
					$isValid = false;
				}
			}

			if (!$isValid) {
				throw EB::exception('There are some required fields which was not provided. Please fill up the form.');
			}
		}
	}
	 ...
	 * @return
	 */
    public function getIntro($stripTags = false, $truncate = true, $source = 'all', $limit = null)
	{
		...
		if ($this->isEbd()) {
			...
			return $doc->getIntro($stripTags, $limit);
		}
	...
	{
		$url = EBR::getRoutedURL('index.php?option=com_easyblog&view=entry&layout=preview&uid=' . $this->id . '.' . $this->revision_id, $xhtml, true);
	...
	{
		$url = 'index.php?option=com_easyblog&view=entry&id=' . $this->id . '&tmpl=component&print=1&format=print';
		$url = EBR::_($url, false);

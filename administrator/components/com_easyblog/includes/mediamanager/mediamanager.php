...
class EBMM extends EasyBlog
{
	...
	public static function render()
	{
		...
		$uploadUrl = JURI::base() . '?option=com_easyblog&task=media.upload&tmpl=component&lang=en&&sessionid=' . $session->getId() . '&' . EB::getToken() . '=1';
	...
	public static function renderFolder($uri)
	{
		...
		// Set the upload url
		$session = JFactory::getSession();
		$uploadUrl = JURI::base() . '?option=com_easyblog&task=media.upload&tmpl=component&lang=en&key=' . $folder->key . '&sessionid=' . $session->getId() . '&' . EB::getToken() . '=1';

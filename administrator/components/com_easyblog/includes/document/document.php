...
class EasyBlogDocument extends EasyBlog
{
	...
	public function hasReadmore()
	{
		...
		if ($this->config->get('composer_truncation_enabled')) {
			...
			// If there are more blocks than the configured truncation
			if (count($blocks) > $max) {
				return true;
			}
	 ...
	 * @return
	 */
	public function getIntro($stripTags = false, $limit = null)
	{
		$output = '';
		...
		if ($this->config->get('composer_truncation_enabled') || $limit) {
			...
			if ($this->config->get('composer_truncate_image_position') == 'top') {
				$output = $imageHTML . $output;
			}

			if ($this->config->get('composer_truncate_image_position') == 'bottom') {
				$output = $output . $imageHTML;
			}

			if ($this->config->get('composer_truncate_video_position') == 'top') {
				$output = $videoHTML . $output;
			}

			if ($this->config->get('composer_truncate_video_position') == 'bottom') {
				$output = $output . $videoHTML;
			}

			if ($this->config->get('composer_truncate_audio_position') == 'top') {
				$output = $audioHTML . $output;
			}

			if ($this->config->get('composer_truncate_audio_position') == 'bottom') {

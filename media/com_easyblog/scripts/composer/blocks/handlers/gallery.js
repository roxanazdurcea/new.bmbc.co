...
EasyBlog.Controller("Composer.Blocks.Handlers.Gallery", {
    ...
    deconstruct: function(block) {
        ...
        // Remove placeholder
        self.galleryItemPlaceholder.inside(galleryContainer)
            .remove();

        // Remove placeholder
        self.galleryMenuItemPlaceholder.inside(galleryContainer)
            .remove();

        var primaryGalleryItem =
            self.galleryItem.inside(galleryContainer)
                .removeClass("active")
        ...
        if (galleryItemIndex < 0) galleryItemIndex = 1;

        // Set viewport position
        var galleryViewport = self.galleryViewport.inside(galleryContainer);
        var left = 100 * -1 * (galleryItemIndex || 0);
    ...
    createGalleryMenuItem: function() {
        return $(meta.galleryMenuItem);
    },
    ...
    "{galleryUploadPlaceholder} mediaUploaderFilesAdded": function(galleryUploadPlaceholder, event, uploader, files) {
        ...
        $.each(files, function(i, file) {
            ...
            var galleryMenuItem = self.createGalleryMenuItem();
            galleryMenuItem
                .attr("data-id", itemId)
    ...
    populateList: function(block) {
        ...
        galleryItems.each(function(){
            ...
            galleryListItem
                .toggleClass("is-primary", itemId==data.primary)
    ...
    "{listItem} click": function(listItem) {
        ...
        var galleryItemIndex = self.getGalleryItemIndex(galleryItem);
    ...
    getGalleryItemIndex: function(galleryItem) {

        var galleryViewport = self.galleryViewport.of(galleryItem);

        return galleryViewport.find(self.galleryItem.selector).index(galleryItem)
    },
